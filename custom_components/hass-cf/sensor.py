...
from espn-api import Schedule, Scoring, Linescore, Broadcasts
...
DEFAULT_NAME = 'CF Sensor'

LOGO_URL = 'https://www.sportslogos.net/logos/view/546390412021/National_Collegiate_Athletic_Association/2021/Primary_Logo'
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    """Set up the CF API sensor."""
    team_id = config.get(CONF_ID)
    name = config.get(CONF_NAME, DEFAULT_NAME)
    scan_interval = config.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)
    add_entities([CFSensor(team_id, name, scan_interval, hass)])
    
    
class CFSensor(Entity):
    """Representation of a CF API sensor."""

    def __init__(self, team_id, name, scan_interval, hass):
        """Initialize CF API sensor."""
    ...
    def get_game_data(self):
        """Get the latest data from the CF API via espn-api."""
        # Get game info
